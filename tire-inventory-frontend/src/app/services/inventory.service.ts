...
//     console.log('Payload sent to backend:', item); // Log the payload
//     return this.http.post<InventoryItem>(this.apiUrl, item, { headers: this.getHeaders() })
//       .pipe(
//         tap((response: InventoryItem) => console.log('Response from backend:', response)),
//         catchError((error) => {
...
export class InventoryService {
  private apiUrl = 'http://localhost:8000/api/inventory';
  private openTireApiUrl =
    'https://raw.githubusercontent.com/yourgithub/opentire/main/data/tires.json';
  ...
  addItem(item: InventoryItem): Observable<InventoryItem> {
    console.log('Payload sent to backend:', item);
    return this.http
      .post<InventoryItem>(this.apiUrl, item, { headers: this.getHeaders() })
      .pipe(
        tap((response: InventoryItem) =>
